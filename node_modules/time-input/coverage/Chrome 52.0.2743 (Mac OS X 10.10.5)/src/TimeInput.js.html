...
<html lang="en">
<head>
    <title>Code coverage report for src/TimeInput.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">all files</a> / <a href="index.html">src/</a> TimeInput.js
    </h1>
    <div class='clearfix'>
...
190
191
192</td><td class="line-coverage quiet"><span class="cline-any cline-yes">5×</span>
...
&nbsp;
var TimeInput = React.createClass(<span class="fstat-no" title="function not covered" >{</span>
  getInitialState () {
    return {}
  },
  getDefaultProps () {
...
<span class="cstat-no" title="statement not covered" >    t</span>his.mounted = false
  },
  componentDidUpdate () {
  ...
  handleBlur () {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.mounted) this.setState({ caretIndex: null })
  },
  handleEscape () {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.mounted) this.input.blur()
  },
  handleTab (event) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.mounted) this.setState({ caretIndex: index })
  },
  handleArrows (event) {
    event.preventDefault()
    var start = caret.start(this.input)
  ...
  handleForwardspace (event) {
    ...
    if (!diff) {
      ...
      while (diff--) {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (value[end - 1] !== ':') {
          value = replaceCharAt(value, start, defaultValue.charAt(start))
        }
        start++
      }
  ...
  handleKeyDown (event) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (event.which === 27) return this.handleEscape(event)
  },
  isSeparator (char) {
    return /[:\s]/.test(char)
  },
    ...
    if (diff &gt; 0) {
      ...
      while (diff--) {
        ...
        if (this.isSeparator(oldChar)) {
          <span class="missing-if-branch" title="if path not taken" >I</span>if (this.isSeparator(newChar)) {
<span class="cstat-no" title="statement not covered" >            i</span>nsertion = insertion.slice(1)
<span class="cstat-no" title="statement not covered" >            s</span>tart++
          } else {
            start++
            diff++
            end++
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (validate(newValue)) {
      this.onChange(newValue, end)
    } else {
<span class="cstat-no" title="statement not covered" >      v</span>ar caretIndex = this.props.value.length - (newValue.length - end)
<span class="cstat-no" title="statement not covered" >      if (this.mounted) <span class="cstat-no" title="statement not covered" >t</span></span>his.setState({ caretIndex: caretIndex })
    }
  },
  onChange: function (str, caretIndex) {
    if (this.props.onChange) this.props.onChange(this.format(str))
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.mounted &amp;&amp; typeof caretIndex === 'number') this.setState({ caretIndex: caretIndex })
  }
})
&nbsp;
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Tue Aug 16 2016 01:30:18 GMT+0100 (BST)
</div>
</div>
